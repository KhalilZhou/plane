...
class ContextManager:
    ...
    def build(self, user_message):
        ...
        在 agent 链路里的位置：
        它位于 `Plane.ask()` 的每轮模型调用之前，是“真正发请求给模型”
        ...
        if checkpoint_text:
            # checkpoint 单独成段，避免被 prefix 的预算裁剪掉
        ...
        for entry in reversed(history_entries):
            ...
            if recent:
                ...
                if rendered_entries:
                 ...
                 # 加上 or 1 防范空列表除零风险
                line_count = len(candidate_lines) or 1
                average_available = max(20, (available // line_count) - 1)

                candidate_lines = [_tail_clip(line, average_available) for line in candidate_lines]
                candidate_entries = candidate_lines + rendered_entries
                candidate_rendered = "\n".join(["Transcript:", *candidate_entries])
                if len(candidate_rendered) <= budget:
                    rendered_entries = candidate_entries
    ...
    def _assemble_prompt(self, rendered):
        ...
        blocks = [rendered["prefix"].rendered]
        checkpoint_text = getattr(self, "_checkpoint_text", "")
        if checkpoint_text:
            blocks.append(checkpoint_text)
        blocks.extend(
            [
