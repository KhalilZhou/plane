...
from ..config import load_plane_env, provider_env
...
DEFAULT_CORE_REPORT_PATH = Path("docs/metrics/plane-benchmark-core-report.md")
...
def build_stress_agent_metrics():
    with tempfile.TemporaryDirectory(prefix="plane-metrics-") as temp_dir:
        workspace_root = Path(temp_dir)
        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
        workspace = WorkspaceContext.build(workspace_root)
        store = SessionStore(workspace_root / ".plane" / "sessions")
        agent = Plane(
...
def _run_memory_variant(mode):
    with tempfile.TemporaryDirectory(prefix="plane-memory-experiment-") as temp_dir:
        workspace_root = Path(temp_dir)
        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
...
def _run_memory_task_variant(task, variant):
    with tempfile.TemporaryDirectory(prefix="plane-memory-large-") as temp_dir:
        workspace_root = Path(temp_dir)
        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
    ...
    for history_label, history_count in history_levels:
        for note_label, note_count in note_levels:
            for request_label, request_text in request_levels:
                per_run = []
                for _ in range(repetitions):
                    with tempfile.TemporaryDirectory(prefix="plane-context-matrix-") as temp_dir:
                        workspace_root = Path(temp_dir)
                        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
                        workspace = WorkspaceContext.build(workspace_root)
                        store = SessionStore(workspace_root / ".plane" / "sessions")
...
def _scenario_symlink_escape(workspace_root):
    ...
    try:
        ...
        # Windows 上创建符号链接需要管理员权限或开发者模式，此环境下跳过该场景
        return {
            "tool_status": "skipped",
            "tool_error_code": "",
            "security_event_type": "",
            ...
            "diff_summary": [],
            "environment_skip": "symlink_not_permitted",
        }
    agent = _security_agent(workspace_root)
...
def run_security_experiment_suite(repetitions=3):
    ...
    environment_skip_counts = {}
    for scenario_id, runner in SECURITY_SCENARIOS:
        for _ in range(repetitions):
            with tempfile.TemporaryDirectory(prefix="plane-security-exp-") as temp_dir:
                workspace_root = Path(temp_dir)
                (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
                ...
                skip = str(metadata.get("environment_skip", "")).strip()
                if skip:
                    environment_skip_counts[skip] = environment_skip_counts.get(skip, 0) + 1
    return {
        "scenario_count": len(SECURITY_SCENARIOS),
        "runs": len(rows),
        "security_event_counts": security_event_counts,
        "tool_error_code_counts": tool_error_code_counts,
        "environment_skip_counts": environment_skip_counts,
        "rows": rows,
    }
...
def _provider_profile(provider):
    ...
    if provider == "gpt":
        ...
        if not api_key:
            return {"provider": provider, "status": "blocked", "reason": "PLANE_OPENAI_API_KEY or OPENAI_API_KEY missing"}
        return {
            "provider": provider,
            "status": "ready",
            "model": provider_env("PLANE_OPENAI_MODEL", ("OPENAI_MODEL",), "gpt-5.6-terra"),
            "base_url": provider_env("PLANE_OPENAI_API_BASE", ("OPENAI_API_BASE",), "https://api.openai.com/v1"),
            "api_key": api_key,
        }
    if provider == "deepseek":
        api_key = provider_env("PLANE_DEEPSEEK_API_KEY", ("DEEPSEEK_API_KEY",))
        if not api_key:
            return {"provider": provider, "status": "blocked", "reason": "PLANE_DEEPSEEK_API_KEY or DEEPSEEK_API_KEY missing"}
        return {
            "provider": provider,
            "status": "ready",
            "model": provider_env("PLANE_DEEPSEEK_MODEL", ("DEEPSEEK_MODEL",), "deepseek-v4-pro"),
            "base_url": provider_env("PLANE_DEEPSEEK_API_BASE", ("DEEPSEEK_API_BASE",), "https://api.deepseek.com/anthropic"),
            "api_key": api_key,
        }
    api_key = provider_env(
    ...
    if not api_key:
        return {"provider": "claude", "status": "blocked", "reason": "PLANE_ANTHROPIC_API_KEY or ANTHROPIC_API_KEY missing"}
    return {
        "provider": "claude",
        "status": "ready",
        "model": provider_env("PLANE_ANTHROPIC_MODEL", ("ANTHROPIC_MODEL",), "claude-sonnet-4-6"),
        "base_url": provider_env("PLANE_ANTHROPIC_API_BASE", ("ANTHROPIC_API_BASE",), "https://api.anthropic.com/v1"),
        "api_key": api_key,
    }
...
def run_real_memory_experiment(provider="gpt", repetitions=1):
    ...
    for task in MEMORY_EXPERIMENT_TASKS:
        ...
        for _ in range(repetitions):
            for variant in variants:
                with tempfile.TemporaryDirectory(prefix="plane-real-memory-") as temp_dir:
                    workspace_root = Path(temp_dir)
                    (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
                    _write_memory_task_files(workspace_root, task)
            ...
            for request_label, request_text in request_levels:
                ...
                for _ in range(repetitions):
                    for variant_name, updates in (("full", {}), ("no_context_reduction", {"context_reduction": False})):
                        with tempfile.TemporaryDirectory(prefix="plane-real-context-") as temp_dir:
                            workspace_root = Path(temp_dir)
                            (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
                            agent = _build_real_agent(workspace_root, provider)
                            for index in range(note_count):
...
def _run_real_repeated_call_scenario(provider):
    with tempfile.TemporaryDirectory(prefix="plane-real-security-repeat-") as temp_dir:
        workspace_root = Path(temp_dir)
        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
        agent = _build_real_agent(workspace_root, provider)
...
def run_real_security_experiment_suite(provider="gpt", repetitions=1):
    ...
    for _ in range(repetitions):
        ...
        for scenario in REAL_SECURITY_SCENARIOS:
            with tempfile.TemporaryDirectory(prefix="plane-real-security-") as temp_dir:
                workspace_root = Path(temp_dir)
                if not _setup_real_security_workspace(workspace_root, scenario["id"]):
                    rows.append(
                        _security_result_row(
                            scenario["id"],
                            provider,
                            {"tool_status": "skipped", "environment_skip": "symlink_not_permitted"},
                        )
                    )
                    continue
    ...
    for row in rows:
        ...
        if error_code:
            tool_error_code_counts[error_code] = tool_error_code_counts.get(error_code, 0) + 1
        skip = str(row.get("environment_skip", "")).strip()
        if skip:
            environment_skip_counts[skip] = environment_skip_counts.get(skip, 0) + 1
...
def render_large_scale_experiment_report(metrics):
    ...
    lines = [
        "# Plane Large-Scale Experiment Report",
        "",
        "## Executive Summary",
        (
...
def _run_recovery_task_variant(task, variant):
    with tempfile.TemporaryDirectory(prefix="plane-recovery-ablation-") as temp_dir:
        workspace_root = Path(temp_dir)
        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
