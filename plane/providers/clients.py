...
OPENAI_COMPATIBLE_USER_AGENT = "plane/0.1"
...
class OpenAICompatibleModelClient:
    def __init__(self, model, base_url, api_key, temperature, timeout):
        ...
        # 官方 OpenAI 端点默认支持缓存；自建网关可用 PLANE_OPENAI_PROMPT_CACHE=1 显式开启。
        self.supports_prompt_cache = bool(os.environ.get("PLANE_OPENAI_PROMPT_CACHE")) or "openai.com" in self.base_url
        self.last_completion_metadata = {}
    ...
    def complete(self, prompt, max_new_tokens, prompt_cache_key=None, prompt_cache_retention=None):
        ...
        在 agent 链路里的位置：
        它位于 `Plane.ask()` 的模型调用阶段，是稳定前缀缓存复用链路真正
        落到 provider API 的地方。
        """
        self.last_completion_metadata = {}
        payload = {
