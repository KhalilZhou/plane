...
class Plane:
    def __init__(
        ...
        self.run_store = run_store or RunStore(Path(workspace.repo_root) / ".plane" / "runs")
    ...
    def parse(raw):
        ...
        if "<tools>" in raw and ("<final>" not in raw or raw.find("<tools>") < raw.find("<final>")):
            body = Plane.extract(raw, "tools")
            try:
                payload = json.loads(body)
            except json.JSONDecodeError:
                return "retry", Plane.retry_notice("<tools> body must be valid JSON")
            if not isinstance(payload, dict) or "calls" not in payload:
                return "retry", Plane.retry_notice('<tools> must contain {"calls": [...]}')
            calls = payload["calls"]
            if not isinstance(calls, list) or not calls:
                return "retry", Plane.retry_notice("calls must be a non-empty array")
            for i, call in enumerate(calls):
                if not isinstance(call, dict):
                    return "retry", Plane.retry_notice(f"call at index {i} must be a JSON object")
                if not str(call.get("name", "")).strip():
                    return "retry", Plane.retry_notice(f"call at index {i} is missing a tool name")
                args = call.get("args", {})
                if args is None:
                    call["args"] = {}
                elif not isinstance(args, dict):
                    return "retry", Plane.retry_notice(f"call at index {i} has invalid args")
            return "tools", {"calls": calls}
        if "<tool>" in raw and ("<final>" not in raw or raw.find("<tool>") < raw.find("<final>")):
            body = Plane.extract(raw, "tool")
            try:
                payload = json.loads(body)
            except Exception:
                return "retry", Plane.retry_notice("model returned malformed tool JSON")
            if not isinstance(payload, dict):
                return "retry", Plane.retry_notice("tool payload must be a JSON object")
            if not str(payload.get("name", "")).strip():
                return "retry", Plane.retry_notice("tool payload is missing a tool name")
            args = payload.get("args", {})
            if args is None:
                payload["args"] = {}
            elif not isinstance(args, dict):
                return "retry", Plane.retry_notice()
            return "tool", payload
        if "<tool" in raw and ("<final>" not in raw or raw.find("<tool") < raw.find("<final>")):
            payload = Plane.parse_xml_tool(raw)
            if payload is not None:
                return "tool", payload
            return "retry", Plane.retry_notice()
        if "<final>" in raw:
            final = Plane.extract(raw, "final").strip()
            if final:
                return "final", final
            return "retry", Plane.retry_notice("model returned an empty <final> answer")
        raw = raw.strip()
        if raw:
            return "final", raw
        return "retry", Plane.retry_notice("model returned an empty response")
    ...
    def parse_xml_tool(raw):
        ...
        if not match:
            return None
        attrs = Plane.parse_attrs(match.group("attrs"))
        ...
        for key in ("content", "old_text", "new_text", "command", "task", "pattern", "path"):
            if f"<{key}>" in body:
                args[key] = Plane.extract_raw(body, key)
