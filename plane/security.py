...
def shell_env(env=None, allowlist=(), root="."):
    ...
    # Windows 上启动 shell 需要这些系统变量，否则 subprocess(shell=True) 会找不到 cmd.exe
    for name, value in env.items():
        if str(name).upper() in {"COMSPEC", "SYSTEMROOT", "PATHEXT"} and str(name) not in filtered:
            filtered[str(name)] = value
