...
from plane.evaluation.evaluator import (
    BenchmarkEvaluator,
    load_benchmark,
...
def test_run_fixed_benchmark_reports_metadata_and_success_definition(tmp_path):
    ...
    assert reproducibility["locale"]  # 平台相关：Linux 下通常为 C.UTF-8，Windows 下为本地代码页
...
def test_run_task_anchors_paths_to_fixture_copy_under_workspace_root(tmp_path):
    # 用临时目录作为 workspace_root：既验证"行里的相对路径锚定在 workspace 内"，
    # 又不会把 fixture 副本与运行工件写进项目目录。
    workspace_root = tmp_path / "workspaces"
    evaluator = BenchmarkEvaluator(
        benchmark_path=Path("benchmarks/coding_tasks.json"),
        artifact_path=tmp_path / "benchmark-v1.json",
    ...
    assert row["status"] == "pass"
    assert not Path(row["fixture_copy_relpath"]).is_absolute()
    fixture_copy = (workspace_root / row["fixture_copy_relpath"]).resolve()
    assert workspace_root.resolve() in fixture_copy.parents
