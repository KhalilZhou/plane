...
from plane.evaluation.metrics import (
    _provider_profile,
...
def test_provider_profile_loads_plane_config_before_reading_deepseek_config(tmp_path, monkeypatch):
    ...
    assert profile["api_key"] == "sk-plane-deepseek"
    assert profile["model"] == "deepseek-v4-pro"
    assert profile["base_url"] == "https://api.deepseek.com/anthropic"


def test_provider_profile_uses_openai_key_for_gpt(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
...
def test_write_benchmark_core_report_marks_resume_safe_metrics(tmp_path):
    ...
    report_path = tmp_path / "docs" / "metrics" / "plane-benchmark-core-report.md"
