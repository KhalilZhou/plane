...
def build_agent(tmp_path, outputs, **kwargs):
    ...
    store = SessionStore(tmp_path / ".plane" / "sessions")
...
def test_list_files_hides_internal_agent_state(tmp_path):
    agent = build_agent(tmp_path, [])
    (tmp_path / ".plane").mkdir(exist_ok=True)
...
def test_openai_compatible_client_posts_expected_responses_payload():
    ...
    assert captured["headers"]["User-agent"] == "plane/0.1"
...
def test_build_agent_uses_openai_provider_and_model_override(tmp_path):
    ...
    with patch.dict(
        ...
        with patch(
            "plane.cli.OllamaModelClient",
            side_effect=AssertionError("ollama client should not be used"),
        ), patch("plane.cli.OpenAICompatibleModelClient") as mock_openai:
...
def test_build_agent_uses_openai_key_for_openai_provider(tmp_path):
    args = type(
        "Args",
        (),
        {
    ...
    with patch.dict(os.environ, {"PLANE_OPENAI_API_KEY": "sk-openai"}, clear=True):
        with patch(
            "plane.cli.OllamaModelClient",
            side_effect=AssertionError("ollama client should not be used"),
        ), patch("plane.cli.OpenAICompatibleModelClient") as mock_openai:
    ...
    mock_openai.assert_called_once()
    assert mock_openai.call_args.kwargs["api_key"] == "sk-openai"
    assert agent.model_client is fake_client


def test_build_arg_parser_leaves_provider_unset_by_default(tmp_path):
    args = plane_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path)])

    assert args.provider is None


def test_build_agent_falls_back_to_deepseek_without_credentials(tmp_path, monkeypatch):
    (tmp_path / "README.md").write_text("demo\n", encoding="utf-8")
    for name in list(os.environ):
        if name == "PLANE_PROVIDER" or name.endswith(("API_KEY", "API_TOKEN", "AUTH_TOKEN")):
...
def test_build_arg_parser_accepts_anthropic_provider(tmp_path):
    args = plane_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "anthropic"])

    assert args.provider == "anthropic"


def test_build_arg_parser_accepts_deepseek_provider(tmp_path):
    args = plane_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "deepseek"])

    assert args.provider == "deepseek"


def test_build_agent_uses_anthropic_provider_and_anthropic_key(tmp_path):
    args = type(
        "Args",
        (),
        {
    ...
    with patch.dict(
        ...
        with patch(
            ...
            "plane.cli.OpenAICompatibleModelClient",
            side_effect=AssertionError("openai client should not be used"),
        ), patch("plane.cli.AnthropicCompatibleModelClient") as mock_anthropic:
...
def test_build_agent_uses_anthropic_default_model_when_env_is_missing(tmp_path):
    ...
    with patch.dict(
        ...
        with patch("plane.cli.AnthropicCompatibleModelClient") as mock_anthropic:
            plane_pkg.build_agent(args)

    assert mock_anthropic.call_args.kwargs["model"] == "claude-sonnet-4-6"


def test_build_agent_uses_deepseek_provider_and_plane_config(tmp_path, monkeypatch):
    """deepseek 的配置来自 plane 自己的配置文件，而不是工作目录里的 .env。"""
    plane_env = tmp_path / "plane.env"
    plane_env.write_text(
        "\n".join(
            [
                "PLANE_DEEPSEEK_API_BASE=https://api.deepseek.com/anthropic",
                "PLANE_DEEPSEEK_API_KEY=sk-plane-deepseek",
                "PLANE_DEEPSEEK_MODEL=deepseek-v4-pro",
            ]
        )
        + "\n",
        encoding="utf-8",
    ...
    assert mock_anthropic.call_args.kwargs["api_key"] == "sk-plane-deepseek"
    assert agent.model_client is fake_client
...
def test_run_shell_nonzero_with_workspace_change_is_recorded_as_partial_success(tmp_path):
    ...
    result = agent.run_tool(
        "run_shell",
        {
            "command": "echo changed > README.md && exit 1",
            "timeout": 20,
        },
    )
...
def test_explicit_memory_promotion_persists_durable_memory_topics(tmp_path):
    agent = build_agent(
        ...
        [
            ...
            "Project convention: Preserve local agent state under .plane/.\n"
    ...
    index_path = tmp_path / ".plane" / "memory" / "MEMORY.md"
    conventions_path = tmp_path / ".plane" / "memory" / "topics" / "project-conventions.md"
    decisions_path = tmp_path / ".plane" / "memory" / "topics" / "key-decisions.md"
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    ...
    assert report["durable_promotions"] == [
        ...
        "project-conventions: Preserve local agent state under .plane/.",
...
def test_explicit_memory_promotion_rejects_secret_shaped_and_transient_lines(tmp_path):
    ...
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    conventions_path = tmp_path / ".plane" / "memory" / "topics" / "project-conventions.md"
    dependency_path = tmp_path / ".plane" / "memory" / "topics" / "dependency-facts.md"
...
def test_public_api_exports_resolve_through_package_path():
    ...
    assert Path(plane_pkg.__file__).as_posix().endswith("/plane/__init__.py")


def test_reviewer_skeleton_docs_exist():
    review_pack = Path("docs/review-pack/evaluation.md")
...
def test_package_import_surface_includes_cli_entrypoints():
    assert callable(plane_pkg.main)
    assert callable(plane_pkg.build_agent)
    assert callable(plane_pkg.build_arg_parser)


def test_module_execution_help_works():
    result = subprocess.run(
        [sys.executable, "-m", "plane", "--help"],
        capture_output=True,
        text=True,
    )
