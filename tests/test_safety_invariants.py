...
from plane import FakeModelClient, Plane, SessionStore, WorkspaceContext
from plane import cli as plane_cli
from plane.task_state import TaskState
...
def test_symlink_path_traversal_is_rejected(tmp_path):
    ...
    try:
        ...
        pytest.skip("当前环境不允许创建符号链接（Windows 需要管理员权限或开发者模式）")
    agent = build_agent(tmp_path, [])
...
def test_cli_build_agent_wires_secret_env_names_from_parser(tmp_path):
    ...
    with patch.dict(os.environ, {"GITHUB_PAT": "ghp-1", "GH_PAT": "ghp-2"}, clear=True), patch(
        ...
        args = plane_cli.build_arg_parser().parse_args(
            [
                "--cwd",
                str(tmp_path),
                "--approval",
        ...
        agent = plane_cli.build_agent(args)
...
def test_cli_build_agent_uses_default_configured_secret_names(tmp_path):
    ...
    with patch.dict(os.environ, {"GH_PAT": "ghp-default-1"}, clear=True), patch(
        ...
        args = plane_cli.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--approval", "auto"])
        agent = plane_cli.build_agent(args)
        assert agent.secret_env_summary()["secret_env_names"] == ["GH_PAT"]


def test_cli_build_agent_loads_plane_config_secrets_before_redaction_setup(tmp_path, monkeypatch):
    class DummyModelClient:
        def __init__(self, *args, **kwargs):
            self.args = args
            self.kwargs = kwargs
    ...
    plane_env.write_text("PLANE_DEEPSEEK_API_KEY=sk-plane-secret\n", encoding="utf-8")
    monkeypatch.setattr(plane_config, "plane_config_paths", lambda: [plane_env])
    with patch.dict(os.environ, {}, clear=True), patch("plane.cli.AnthropicCompatibleModelClient", DummyModelClient):
        args = plane_cli.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "deepseek"])
        agent = plane_cli.build_agent(args)
        assert agent.secret_env_summary()["secret_env_names"] == ["PLANE_DEEPSEEK_API_KEY"]
...
def test_cli_build_agent_reads_secret_names_from_environment_config(tmp_path):
    ...
    with patch.dict(
        os.environ,
        {
            "PLANE_CUSTOM_SECRET": "custom-secret-value",
            "PLANE_SECRET_ENV_NAMES": "PLANE_CUSTOM_SECRET",
        },
        clear=True,
    ), patch("plane.cli.OllamaModelClient", DummyModelClient):
        args = plane_cli.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--approval", "auto"])
        agent = plane_cli.build_agent(args)
        assert agent.secret_env_summary()["secret_env_names"] == ["PLANE_CUSTOM_SECRET"]


def test_run_shell_uses_allowlisted_environment_only(tmp_path):
    secret = "shh-allowlist-secret"
    agent = build_agent(tmp_path, [], approval_policy="auto")
    script = "import os; print(os.getenv('PLANE_ALLOWLIST_SECRET', 'missing'))"
    command = f'"{sys.executable}" -c "{script}"'

    with patch.dict(os.environ, {"PLANE_ALLOWLIST_SECRET": secret}, clear=False):
...
def test_bound_tool_methods_delegate_into_tools_module(tmp_path):
    ...
    with patch("plane.tools.subprocess.run") as fake_run:
    ...
    assert agent.tool_run_shell.__func__.__module__ == "plane.runtime"

    with patch("plane.tools.tool_delegate", return_value="toolkit-delegate") as fake_delegate:
